...
- `resolveProject` also returns `originKind` ('pipeline' when it resolved via the worktree-aware or git-origin-walk path, 'direct' otherwise), persisted once as `sessions.origin_kind` alongside `project_id` (sticky, same as project_id). This is what distinguishes a worktree/verification-pipeline-launched session (no-mistakes or similar) from a direct interactive one in the UI (`OriginKindBadge`) and in the per-project cost split (`ProjectCostSummary.bySource`) — keep both in sync with the resolver if you add a new pipeline-style launch path
- Token/cost data has one computation path (`app/server/src/transcript-parser/index.ts`, per-session, on demand) and two consumers layered on top of it: the session stats modal (`routes/transcript-stats.ts`, uncached) and the constellation view's per-project well label (`routes/projects.ts`'s `GET /projects/:id/cost-summary`, backed by `transcript-parser/project-cost-summary.ts`, which sums the per-session pipeline across a project, splits the totals by `sessions.origin_kind` into `bySource.pipeline`/`bySource.direct`, and caches the result in-memory with a TTL — same pattern as `models-pricing.ts`). Add new cost/token surfaces on top of the shared computation path, not a new one
