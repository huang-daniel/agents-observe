...
Claude Code Hooks  →  hook.sh  →  durable spool  →  API Server (SQLite)  →  React Dashboard
  (stdin JSON)      (spool write)   (collector consumer)  (parse + store)     (WebSocket live)
```

`hook.sh` writes each event straight to the durable spool and exits (negotiating the spool schema with the live collector — see [collector-supervision.md](./docs/collector-supervision.md)); the long-lived collector (the API server process) consumes it, dispatching to an agent-class-specific lib (`hooks/scripts/lib/agents/<class>.mjs`) whose `buildHookEvent()` constructs the envelope — including agent-class-aware flags like `meta.isNotification` / `meta.clearsNotification` — then commits it to SQLite directly, no HTTP round trip. The server applies those flags mechanically to its stored state (e.g. `pending_notification_ts`) and forwards events to subscribed WebSocket clients; it stays agent-class-neutral. If the collector isn't healthy, `hook.sh` arms it via the lock/heartbeat supervisor (see [collector-supervision.md](./docs/collector-supervision.md)). The React dashboard derives all agent state (status, event counts, timing) from the event stream.
...
**Hooks** fire on every Claude Code event (tool calls, prompts, stops, subagent lifecycle). See [Architecture](#architecture) above for how `hook.sh` spools raw events for the collector to normalize and store. On the legacy direct-CLI fallback path, if the server needs additional data (like the session's human-readable slug), it responds with a request — the hook reads it from the local transcript file and sends it back.
