...
// From inside the container the container-liveness leg is self-evident — this
// code is running — but only for our own instance. Any other container is
// something this process cannot inspect, and the host predicate (which can ask
// docker) is the one that resolves it. That asymmetry is deliberate: see
// docs/collector-supervision.md § "Two collector runtimes".
describe('a containerized collector judging its own data root', () => {
  function claimDocker(instanceId: string, container = 'agents-observe'): void {
    tryClaimLock(
      ...
      testLockOptions({ runtime: 'docker', containerName: container, instanceId }),
    )
  }

  it('is healthy on its own fresh heartbeat, with no PID leg involved', () => {
    claimDocker('inst-docker')
    beat('inst-docker', 4242)
    expect(health('inst-docker').status).toBe('healthy')
  })

  it('reports a stale heartbeat as unhealthy, exactly as a host collector does', () => {
    claimDocker('inst-docker')
    beat('inst-docker', 4242, nowEpoch() - GRACE - 5)
    const result = health('inst-docker')
    expect(result.status).toBe('unhealthy')
    expect(result.reason).toBe('stale-heartbeat')
  })

  it('will not vouch for a container that is not this one', () => {
    claimDocker('somebody-elses-run')
    beat('somebody-elses-run', 4242)
    const result = health('inst-docker')
    expect(result.status).toBe('invalid-owner')
    expect(result.reason).toBe('container-unverifiable')
    expect(healthExitCode(result.status)).toBe(2)
  })

  it('rejects a docker lock with no container recorded', () => {
    claimDocker('inst-docker', '')
    beat('inst-docker', 4242)
    const result = health('inst-docker')
    expect(result.status).toBe('invalid-owner')
    expect(result.reason).toBe('malformed-lock')
  })
})
