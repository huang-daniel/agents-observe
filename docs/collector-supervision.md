...
> start, restart, and stop that collector. Every agent (Claude Code and Codex
> alike) reaches it the same way:
...
The collector is always a Node process on this host, proved alive by its PID
plus start time, executable, and the entrypoint marker in its command line. That
combination is what survives PID reuse: a recycled PID always has a later start
time, so a lock recorded against PID 4242 can never be mistaken for somebody
else's `vim` minutes later.

**A source-only install bootstraps itself.** A Claude plugin marketplace install
is a clone of this repository and nothing else — no `app/server/node_modules`,
no built dashboard. `observe_bootstrap_collector` (in `observe-lifecycle.sh`)
closes that gap on the first start, with the start lock held so concurrent hooks
cannot install over each other:

- **Server dependencies.** `npm ci` when there is a lockfile, falling back to
  `npm install` when `npm ci` refuses (missing or out-of-sync lockfile). Without
  them there is no collector at all, so a failure aborts the start with a
  message naming `runtime/collector-install.log`, rather than degrading into a
  silent absence.
- **The dashboard.** `app/client/dist` is a build artifact, so a fresh install
  has no UI until it is built. A failure here is a *warning*: a collector with
  no `dist/` still captures every event.

Both are one-time costs and both are recorded in the lifecycle ledger
(`install`/`server-deps-*`, `install`/`client-build-*`). The one end-to-end proof
that this works from true zero state is `test/fresh-install/`.
...
| `AGENTS_OBSERVE_INSTALL_TIMEOUT`    | `300`                      | Bootstrap install budget, seconds           |
| `AGENTS_OBSERVE_NPM`                | `npm`                      | Package manager the bootstrap install runs  |
...
Nor is *not being able to see* evidence. An identity that cannot be read at all —
an unreadable `/proc`, a lock still mid-claim inside its settle window — leaves
the owner alone rather than declaring it gone.
...
delivers anything: `kill` to an identity-matched PID and nothing else. There is
no `pkill -f` path — a pattern match would hit every data root's collector on
the machine, not just this one's.
...
- `start` is idempotent: a healthy owner is attached to. Otherwise it waits up
  ...
  until it has *confirmed*, which on a first start (dependencies still to
  install) can be far longer than a peer's whole wait, and failing a start that
  already succeeded turns one slow start into a herd of failures. This can never
  produce a second collector: that path starts nothing. Once it owns the lock it
  checks health again, reclaims only a lock that
  `observe_collector_lock_is_abandoned` proves abandoned, bootstraps a
  source-only checkout if it has to (see [One collector
  runtime](#one-collector-runtime)), and forks one Node entrypoint. It reports
  `started` only after the canonical predicate succeeds for the PID it forked
  (including the optional HTTP health leg).
- `restart` sends `TERM` through `observe_signal_locked_collector`, waits for the
...
The ordering is what keeps the hook path cheap: `hook.sh` evaluates this
predicate on *every* event, and every leg is a file read or a `/proc` read.
...
The block is also the **capability evidence** anything checking for a live
collector should look at. `ok:true` at the expected version is *not* evidence of
a collector: a server built before supervision serves `/api/health` exactly as
well while never claiming the lock or publishing a heartbeat, so the supervisor
could never confirm it. The fresh-install harness asserts the block for exactly
that reason (`test/fresh-install/entrypoint.sh`).
...
| `invalid-owner` | `malformed-lock`          | 2    | No usable PID or recorded identity.                |
