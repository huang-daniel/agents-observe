...
# How long a dependency install (`npm ci` / `npm install`) is given before the
# start attempt gives up on it. A source-only install — a Claude plugin install
# is one — has to fetch the server's dependencies once before the collector can
# run at all, and that is far slower than forking a process.
OBSERVE_INSTALL_TIMEOUT=${AGENTS_OBSERVE_INSTALL_TIMEOUT:-300}
# The package manager that bootstrap install runs. Overridable so a host with a
# differently-named npm, or a test, can point it somewhere else.
OBSERVE_NPM=${AGENTS_OBSERVE_NPM:-npm}
